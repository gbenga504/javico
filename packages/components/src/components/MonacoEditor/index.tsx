...
const MonacoEditor: React.FC<IProps> = ({
  ...
  const createEditor = useCallback(() => {
    const model = monacoRef.current.editor.createModel(defaultValue, language);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      changeSubscriptionRef.current && changeSubscriptionRef.current.dispose();
    };
  }, []);
  ...
  function handleHighlightSelectedValue() {
    colorHighlight();
    onHighlightValue(selectionValue, commentIconRef.current);
  }
